...
class MaskFirst(nn.Module):
    ...
    def forward(self, image, targets=None):
        ...
        for i in range(N):
            ...
            import pdb; pdb.set_trace()
            img_np_1 = image.tensors[0].detach().cpu().permute(1,2,0).numpy()
            img_np_2 = (img_np_1 - img_np_1.min())/(img_np_1.max() - img_np_1.min())*255
            img_np_3 = ((img_np_1 - img_np_1.min())/(img_np_1.max() - img_np_1.min()))[:,:,::-1]
            import cv2
            cv2.imwrite('mac/img_0.jpg', img_np_2)
            import matplotlib.pyplot as plt
            plt.imsave('mac/img_plt_0.jpg', img_np_3)
