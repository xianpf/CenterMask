...
# v23 feature: target interpolate bilinear
class MaskPyramids(nn.Module):
    def __init__(self, cfg):
        ...
        self.conv_class3 = resnet.Bottleneck(256, 64, 256, 1, True, 1, 1, nn.BatchNorm2d)
        ...
        self.chn256_class3 = nn.Conv2d(1024+1, 256, 1)
    ...
    def _init_target_old(self, img_tensor_shape, device, target=None):
    ...
    def compute_class(self, feature, pyramids, target_levels):
        class_losses = []
        for pyramid in pyramids:
            ...
            chn256 = self.chn256_class3(conv_in)
        ...
        class_loss = sum(loss for loss in class_losses) / len(class_losses)\
            if len(class_losses) else class_logits.sum()*0
    ...
    def forward(self, image, targets=None):
        ...
        for i in range(N):            
            ...
            class_loss = self.compute_class(xs_r50[-4], inst_pyramids, target_levels)
...
def main():
    ...
    cfg.merge_from_list(['OUTPUT_DIR', 'run/scratch_v23_city'])
    ...
    cfg.merge_from_list(['DATASETS.TRAIN', ["cityscapes_fine_instanceonly_seg_train_cocostyle",]])
