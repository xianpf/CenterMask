...
# v25 feature: 根据 score 分配target 高者得 有target 没分配的 的踢出bp，
# score 由在双方的root值的和的高低来判断
# nonononono score 预先计算出 由level 3 配合gaussian map 直出
class MaskPyramids(nn.Module):
    def __init__(self, cfg):
        ...
        self.multi_cs_criteron = nn.CrossEntropyLoss()
    ...
    def compute_mask(self, level, feature, pyramids, is_init=False):
        ...
        for pyramid in pyramids:
            ...
            feature_gaussian_mask = F.interpolate(pyramid.init_gaussian_mask[None,None,...]\
                .to(feature.device), tuple(feature[0,0].shape), mode='bilinear')[0,0]
            if is_init:
    ...
    def compute_loss_v22(self, level, pyramids, target_levels, target_support_pyramids):
    ...
    def compute_loss_v24_1(self, level, pyramids, target_levels, target_support_pyramids):
        # TODO: multi target cross entropy loss
        # 策略：同个target 直接相加取平均， 最后test NMS 或merge
        # pry_targets = list(set([pyr.target_idx for pyr in pyramids]))
        pyr_targets_nonone = sorted(list(set([pyr.target_idx for pyr in pyramids if pyr.target_idx is not None])))
        # print('mentioned target ids:', list(set(pry_targets)))
        # print('target sum:', target_levels[7-level].sum(), 'per target occupy:', \
        #     target_levels[7-level][0].sum(-1).sum(-1))

        # target_here = target_levels[7-level]

        masks_for_loss = []
        targets_for_loss = torch.zeros_like(target_levels[7-level][0,0])
        masks_none = torch.cat([pyr.get_mask(level)[:,1] for pyr in pyramids if pyr.target_idx\
             is None]).mean(0)[None, None]
        masks_for_loss.append(masks_none)
        # import pdb; pdb.set_trace()
        # 修改遍历目标 来 容纳 未被覆盖target
        for i, target_idx in enumerate(pyr_targets_nonone):
            masks_target_idx = torch.cat([pyr.get_mask(level)[:,1] for pyr in pyramids \
                if pyr.target_idx==target_idx]).mean(0)[None, None]
            masks_for_loss.append(masks_target_idx)
            targets_for_loss[(target_levels[7-level][0, target_idx] > 0)] = i+1

            if i >= 70:
                import pdb; pdb.set_trace()
        
        masks_for_loss = torch.cat(masks_for_loss, dim=1)
        targets_for_loss = targets_for_loss[None]
        # import pdb; pdb.set_trace()
        loss_cs = self.multi_cs_criteron(masks_for_loss, targets_for_loss.long())

        return loss_cs

    def compute_loss_v24_2(self, level, pyramids, target_levels, target_support_pyramids):
        # TODO: multi target cross entropy loss
        # 策略：同个target 直接相加取平均， 最后test NMS 或merge
        masks_for_loss = []
        targets_for_loss = torch.zeros_like(target_levels[7-level][0,0])
        masks_none_list = [pyr.get_mask(level)[:,1] for pyr in pyramids if pyr.target_idx is None]
        if masks_none_list:
        ...
        for target_idx in range(target_levels[7-level].shape[1]):
            pyr_mask_list = [pyr.get_mask(level)[:,1] for pyr in pyramids if pyr.target_idx==target_idx]
            if pyr_mask_list:
                masks_target_idx = torch.cat(pyr_mask_list).mean(0)[None, None]
                masks_for_loss.append(masks_target_idx)
            else:
                masks_for_loss.append(torch.zeros_like(masks_none))
            targets_for_loss[(target_levels[7-level][0, target_idx] > 0)] = target_idx+1
            if target_idx >= 70:
                import pdb; pdb.set_trace()
    ...
    def compute_loss(self, level, pyramids, target_levels, target_support_pyramids):
        ...
        for target_idx in range(target_levels[7-level].shape[1]):
            pyr_mask_list = [pyr.get_mask(level)[:,1] for pyr in pyramids if pyr.target_idx==target_idx]
            pyr_score_list = [pyr.get_mask(level)[:,1] for pyr in pyramids if pyr.target_idx==target_idx]
            if pyr_mask_list:
                import pdb; pdb.set_trace()
                masks_target_idx = torch.cat(pyr_mask_list).mean(0)[None, None]
                masks_for_loss.append(masks_target_idx)
    ...
    def l3_merge_loss(self, level, pyramids, target_levels, target_support_pyramids):
        # TODO: multi target cross entropy loss
        # 策略：同个target 直接相加取平均， 最后test NMS 或merge
        import pdb; pdb.set_trace()
        pyr_targets = [pyr.target_idx for pyr in pyramids]
        losses = []
        miss_losses = []
        # class_losses = []
        # covered_idx = []
    ...
    def forward(self, image, targets=None):
        ...
        for i in range(N):            
            ...
            # merge_loss = self.l3_merge_loss(3, inst_pyramids, target_levels, target_support_pyramids)
...
class InstancePyramid():
    ...
    def merge_other_pyr(self, pyr):
        self.merged_others += pyr.merged_others + [pyr.idx,]
        pyr.is_alive = False
    ...
    #         .to(feature_c0.device), tuple(feature_c0.shape), mode='bilinear')[0,0]
...
def main():
    ...
    cfg.merge_from_list(['OUTPUT_DIR', 'run/scratch_v24'])
