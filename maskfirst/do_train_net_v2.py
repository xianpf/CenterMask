...
class MaskFirst(nn.Module):
    def __init__(self, cfg):
        ...
        self.mask_7_6_conv = nn.Conv2d(256+1, 1, 3, padding=1)
        # 考虑改用mask score rcnn
        # self.score = nn.Conv2d(256+1, 1, 1)
        # self.score = nn.Linear(256+1, 1, 1)
    ...
    def forward(self, image, targets=None):
        ...
        for i in range(N):
            ...
            pyramids_7 = self.compute_mask_7(init_inst_pyramids, fs_fpn_p7[i])
            loss7 = self.loss_of_pyramid_7(pyramids_7, target_levels)
            # losses.append(torch.tensor(loss7, device=x_img.device).sum())
            losses.append(sum(loss for loss in loss7))
            pyramids_6 = self.compute_mask_6(pyramids_7, fs_fpn[3][i])
    ...
    def compute_mask_6(self, pyramids, fs_fpn_p6_i):
        # import pdb; pdb.set_trace()
        # losses = []
        for pyramid in pyramids:
            # pyramid._init_mask(fs_fpn_p7_i, self.mask_init_conv)
            mask_7 = pyramid.masks[7]
            # import pdb; pdb.set_trace()
            up_size_6 = tuple(fs_fpn_p6_i.shape[-2:])
            mask_7_up = F.interpolate(mask_7, up_size_6, mode='bilinear', align_corners=False)
            x_conv_6_in = torch.cat((fs_fpn_p6_i.unsqueeze(0), mask_7_up), dim=1)
            mask_6 = torch.sigmoid(self.mask_7_6_conv(x_conv_6_in))
            new_pyramid_pos = torch.nonzero(mask_6[0][0] < 0.2)
            # new_pyramids = self.append_pyramids(new_pyramid_pos, up_size_6, 6)

        return pyramids

    def append_pyramids(self, positions, level_shape, level):
        insts = []
    ...
    def compute_mask_7(self, pyramids, fs_fpn_p7_i):
        # import pdb; pdb.set_trace()
        # losses = []
        for pyramid in pyramids:
            pyramid._init_mask(fs_fpn_p7_i, self.mask_init_conv)
...
class InstancePyramid():
    ...
    def _init_mask(self, feature, mask_init_conv):
        ...
        self.masks[self.init_level] = torch.sigmoid(mask_init_conv(feature_pos))

    # def _score(self, score_conv, mask):
    #     import pdb; pdb.set_trace()
    #     f_0 = torch.zeros_like(feature[0]).unsqueeze(0)
    #     f_0[0, self.pos_h, self.pos_w] = self.score
    #     # self.score = 
