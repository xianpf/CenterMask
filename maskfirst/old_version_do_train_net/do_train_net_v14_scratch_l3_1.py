...
class MaskPyramids(nn.Module):
    ...
    def compute_loss(self, level, pyramids, target_levels, target_support_pyramids):
        ...
        for pyramid in pyramids:
            ...
            if pyramid.target_idx:
                ...
                loss_cs = self.cs_criteron(mask, target_mask.squeeze(1).long())
                losses.append(loss_cs)

                # if loss_cs > 1e20 or torch.isnan(loss_cs):
                if loss_cs > 1e28 or torch.isnan(loss_cs):
                    import pdb; pdb.set_trace()
        ...
        cs_loss = sum(loss for loss in losses)/len(losses)\
            if len(losses) else mask.sum()*0
        miss_loss = sum(loss for loss in miss_losses) / len(miss_losses)\
            if len(miss_losses) else mask.sum()*0
    ...
    def forward(self, image, targets=None):
        ...
        for i in range(N):            
            ...
            if x_curr.min() == float('-inf') or x_curr.max() > 1e28 or torch.isnan(x_curr.max()):
                import pdb; pdb.set_trace()
            # import pdb; pdb.set_trace()
            self.compute_mask(curr_level, x_curr[[i]], inst_pyramids)
            # new_masks_minus = torch.cat([i_p.get_mask(curr_level)[:,[1]] - i_p.get_mask(curr_level)[:,[0]] for i_p in inst_pyramids], dim=1)
            ...
            # if len(new_pos) > 100:
            #     # import pdb; pdb.set_trace()
            #     raw_pos = new_masks_softmax.max(dim=1)[0].view(-1).topk(100, largest=False)[1]
...
def train(cfg, local_rank, distributed):
    ...
    model.load_state_dict(torch.load('run/scratch_v14/model_0040000.pth')['model'])
    # model.load_state_dict(torch.load('run/try_v14_l2/model_0010000.pth')['model'])
...
def main():
    ...
    cfg.merge_from_list(['OUTPUT_DIR', 'run/scratch_v14_l3_1'])
    cfg.merge_from_list(['MODEL.WEIGHT', 'run/try_v14_l2/model_0050000.pth'])
    cfg.freeze()
