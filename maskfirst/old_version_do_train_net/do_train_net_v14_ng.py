    ...
    for iteration, (images, targets, _) in enumerate(data_loader, start_iter):
        ...
        if iteration % 20 == 0 or iteration == max_iter:
            ...
            logger.info("# of instances: level 0: {}\t|level 1: {}\t|level 2: {}\t|level 3: {}\t|level 4: {}\t|level 5: {}".format(
                model.log_dict['pyr_num_l0'], model.log_dict['pyr_num_l1'], 0, 0, 0, 0))
...
# v14 feature: target match every level update
class MaskPyramids(nn.Module):
    ...
    def compute_mask_general(self, level, feature, pyramids, is_init=False):
        for j, pyramid in enumerate(pyramids):
            import pdb; pdb.set_trace()

            feature_size = tuple(feature.shape[-2:])
            gaussian_mask = pyramid.gaussian_masks[level]
            level_pos = pyramid.get_root_level_pos(level)
            ctr = (pyramid.feature_scales[level]*2-1,)*2
    ...
    def forward(self, image, targets=None):
        ...
        for i in range(N):            
            ...
            for i in range(len(inst_pyramids)):
                if i not in merit_pyramids_idx:
                    die_id = inst_pyramids[i].idx
                    die_target_idx = inst_pyramids[i].target_idx
                    if die_target_idx:
...
class InstancePyramid():
    ...
    def __init__(self, pos, init_pub_level, level_sizes):
        ...
        if isinstance(pos, torch.Tensor):
            self.pos = tuple(pos.detach().cpu().tolist())
        else:
            import pdb; pdb.set_trace()
        self.masks = {}
        self.mask_logits = {}
    ...
    def get_root_level_pos(self, pub_level):
        ...
        # import pdb; pdb.set_trace()
        h = round(1.0 * self.pos[0] / init_size[0] * req_size[0])
        w = round(1.0 * self.pos[1] / init_size[1] * req_size[1])
...
def train(cfg, local_rank, distributed):
    ...
    model.load_state_dict(torch.load('run/try_v13/model_0010000.pth')['model'])
