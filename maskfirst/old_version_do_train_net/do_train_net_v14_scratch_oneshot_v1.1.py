...
class MaskPyramids(nn.Module):
    ...
    def forward(self, image, targets=None):
        ...
        for i in range(N):            
            ...
            target_len_3_before = sum([len(l) for l in target_support_pyramids_0])
            for i3 in range(len(inst_pyramids)):
                if i3 not in merit_pyramids_idx:
                    die_id = inst_pyramids[i3].idx
                    die_target_idx = inst_pyramids[i3].target_idx
                    if die_target_idx:
                        target_support_pyramids_0[die_target_idx].remove(die_id)
            target_len_3_after = sum([len(l) for l in target_support_pyramids_0])
            # if target_len_2_before != target_len_2_after:
            #     import pdb; pdb.set_trace()

            inst_pyramids = merit_pyramids + new_pyramids
            self.log_dict.update({'pyr_num_l3': len(inst_pyramids)})
            if self.training:
                # self.match_target(curr_level, new_pyramids, target_levels, target_support_pyramids_0)
                loss_3 = self.compute_loss(curr_level, inst_pyramids, target_levels, target_support_pyramids_0)
            ...
            target_len_4_before = sum([len(l) for l in target_support_pyramids_0])
            for i4 in range(len(inst_pyramids)):
                if i4 not in merit_pyramids_idx:
                    die_id = inst_pyramids[i4].idx
                    die_target_idx = inst_pyramids[i4].target_idx
                    if die_target_idx:
                        target_support_pyramids_0[die_target_idx].remove(die_id)
            target_len_4_after = sum([len(l) for l in target_support_pyramids_0])

            inst_pyramids = merit_pyramids + new_pyramids
            self.log_dict.update({'pyr_num_l4': len(inst_pyramids)})
            if self.training:
                # self.match_target(curr_level, new_pyramids, target_levels, target_support_pyramids_0)
                loss_4 = self.compute_loss(curr_level, inst_pyramids, target_levels, target_support_pyramids_0)
                losses_4.append(loss_4)
            ...
            target_len_5_before = sum([len(l) for l in target_support_pyramids_0])
            for i5 in range(len(inst_pyramids)):
                if i5 not in merit_pyramids_idx:
                    die_id = inst_pyramids[i5].idx
                    die_target_idx = inst_pyramids[i5].target_idx
                    if die_target_idx:
                        target_support_pyramids_0[die_target_idx].remove(die_id)
            target_len_5_after = sum([len(l) for l in target_support_pyramids_0])

            inst_pyramids = merit_pyramids + new_pyramids
            self.log_dict.update({'pyr_num_l5': len(inst_pyramids)})
            if self.training:
                # self.match_target(curr_level, new_pyramids, target_levels, target_support_pyramids_0)
                loss_5 = self.compute_loss(curr_level, inst_pyramids, target_levels, target_support_pyramids_0)
                losses_5.append(loss_5)
