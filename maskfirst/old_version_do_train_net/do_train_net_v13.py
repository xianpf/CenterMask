...
# v13 feature: target match outside, cs/miss loss independent
class MaskPyramids(nn.Module):
    def __init__(self, cfg):
        ...
        self.conv = [self.conv_6, self.conv_5]
        self.out_6 = nn.Conv2d(256, 2, 1)
        self.out_5 = nn.Conv2d(256, 2, 1)
        self.out = [self.out_6, self.out_5]
        ...
        self.chn256 = [self.chn256_6, self.chn256_5]
    ...
    def compute_mask(self, level, feature, pyramids, is_init=False):
        for j, pyramid in enumerate(pyramids):
            ...
            if is_init:
                ...
                out = self.upmix256_5(conv_in)
                mask_logits = self.conv_5(out)
                pyramid.set_mask_logits(level, mask_logits)
                mask = self.out_5(mask_logits)
                pyramid.set_mask(level, mask)
    ...
    def forward(self, image, targets=None):
        ...
        for i in range(N):            
            ...
            if self.training:
                ...
                # for pyr in inst_pyramids:
                    # target_idxs = target_levels[7][0, :, pyr.pos[0], pyr.pos[1]].nonzero()
                    # for i, target_idx in enumerate(target_idxs):
                    #     target_idx_int = target_idx[0].item()
                    #     target_support_pyramids_0[target_idx_int].append(pyr.idx)
                    #     # 解决一个pixel 多个target 的问题， 核心：(小target优先)， 已分配的不改
                    #     if pyr.target_idx:
                    #         # print('target_idxs', target_idxs)
                    #         # print('pyr.target_idx:', pyr.target_idx)
                    #         target_map_last = target_levels[7][0, pyr.target_idx]
                    #         target_map_now = target_levels[7][0, target_idx_int]
                    #         # 重叠的上一个已经有其他pixel选项了， 让其让路
                    #         if len(target_support_pyramids_0[pyr.target_idx]) > 1:
                    #             target_support_pyramids_0[pyr.target_idx].remove(pyr.idx)
                    #             pyr.bind_target(target_idx_int)
                    #         elif (target_map_now == target_map_last).all():
                    #             target_support_pyramids_0[target_idx_int].remove(pyr.idx)
                    #             continue
                    #         elif target_map_now.sum() < target_map_last.sum():
                    #             target_support_pyramids_0[pyr.target_idx].remove(pyr.idx)
                    #             pyr.bind_target(target_idx_int)
            ...
            # new_masks = torch.cat([i_p.get_mask(curr_level)[:,[1]] for i_p in inst_pyramids], dim=1)
            ...
            # pending_thresh = max(0.05, max_topk)
            # pending_thresh = max(avg_sharing*2.0, max_topk)
            pending_thresh = 0.03   #0.05
            ...
            clean_pyramids_idx = new_masks_softmax.topk(3, dim=1)[1].unique()
            clean_pyramids = [inst_pyramids[i] for i in range(len(inst_pyramids)) if i in clean_pyramids_idx]
            inst_pyramids = clean_pyramids + new_pyramids
            if self.training:
                # import pdb; pdb.set_trace() 
                # target_5 = target_levels[7-1]
                # inst_pyramids_targets = [p.target_idx for p in inst_pyramids]
                # inst_pyramids_targets = [p.idx for p in inst_pyramids]
...
def train(cfg, local_rank, distributed):
    ...
    model.load_state_dict(torch.load('run/try_v11.2/model_0060000.pth')['model'])
