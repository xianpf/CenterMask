...
# v22 feature: interpolate bilinear 问题解决
    ...
    def pyr_list_to_boxlist_visual(self, pyramids, image, image_size):
        ...
        for pyr in pyramids:
            ...
            if mask.sum():
                ...
                # import pdb; pdb.set_trace()
                scaled_back_box = box * 16  # 800/50
                valid_boxes.append(scaled_back_box)
                valid_pyramids.append(pyr)

        bg_logits = pyramids[0].get_mask(3)[:,[0]]
        if len(valid_boxes) == 0:
            boxes = mask.new_empty((0,4))
            class_logits = pyramids[0].class_logits
            scores = mask.new_empty((0))
            labels = mask.new_empty((0))
            box_masks = mask.new_empty((0, 1, image_size[0], image_size[1]))
        else:
            ...
            # import pdb; pdb.set_trace()
            # box_masks = F.interpolate(box_masks, scale_factor=16, mode='bilinear')
            # box_masks = box_masks[:,:,:image_size[0], :image_size[1]]   # crop

        boxlist = BoxList(boxes, image_size[::-1], mode="xyxy")
        boxlist.add_field("dataloader_size", torch.tensor(image_size[::-1]))
        boxlist.add_field("bg_logits", bg_logits)
...
class COCODemo(object):
    ...
    def compute_prediction(self, original_image):
        ...
        img_show_0 = prediction.get_field("bg_logits")
        img_show_1 = prediction.get_field("mask")
        img_show = torch.cat((img_show_0, img_show_1))[:2].max(dim=0)[1][0]
        img_show = img_show.detach().cpu().numpy()
...
def main():
    ...
    cfg.merge_from_list(['MODEL.WEIGHT', 'run/scratch_v22/model_0080000.pth'])
    cfg.merge_from_list(['SOLVER.IMS_PER_BATCH', 1])
    cfg.merge_from_list(['TEST.IMS_PER_BATCH', 16])
    cfg.merge_from_list(['DATASETS.TEST', ('coco_2014_microval',)])
