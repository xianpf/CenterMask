    ...
    def forward(self, image, targets=None):
        ...
        for i in range(N):
            ...
            if self.training:
                ...
                # import pdb; pdb.set_trace()
                # masks_0 = torch.cat([i_p.get_mask(0) for i_p in inst_pyramids], dim=0)
                masks_0 = torch.cat([i_p.get_mask(2) for i_p in inst_pyramids], dim=0)
                # # 这里暂且以mask的大小作为原图的大小， 后面要在inference里改回去
                # fake_box = torch.tensor([[100, 50, 200, 150]]*len(masks_0), device=masks_0.device)
                # box_fake = BoxList(fake_box, tuple(masks_0.shape[-2:]), mode="xyxy")
                # box_fake.add_field('mask', masks_0)

                # return [box_fake]
                test_masks.append(masks_0)
